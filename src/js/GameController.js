...
export default class GameController {
  ...
  raiseAttack(character) {
    return Math.floor(Math.max(character.attack, character.attack * (80 + character.health) / 100));
  }

  raiseDefence(character) {
    return Math.floor(Math.max(character.defence, character.defence * (80 + character.health) / 100));
  }
  ...
  onLoadGameClick() {
    ...
    for (let i = 0; i < loadGame.playerTeam.length; i++) {
      let newCharacter;
      if (loadGame.playerTeam[i].character.type === 'bowman') {
        newCharacter = new Bowman(loadGame.playerTeam[i].character.level);
      } else if (loadGame.playerTeam[i].character.type === 'magician') {
        newCharacter = new Magician(loadGame.playerTeam[i].character.level);
      } else if (loadGame.playerTeam[i].character.type === 'swordsman') {
        newCharacter = new Swordsman(loadGame.playerTeam[i].character.level);
      }
      newCharacter.attack = loadGame.playerTeam[i].character.attack;
      newCharacter.defence = loadGame.playerTeam[i].character.defence;
      newCharacter.health = loadGame.playerTeam[i].character.health;
      const positionedCharacter = new PositionedCharacter(newCharacter, loadGame.playerTeam[i].position);
      this.gameState.playerTeam.push(positionedCharacter);
    }

    for (let i = 0; i < loadGame.compTeam.length; i++) {
      let newCharacter;
      if (loadGame.compTeam[i].character.type === 'daemon') {
        newCharacter = new Daemon(loadGame.compTeam[i].character.level);
      } else if (loadGame.compTeam[i].character.type === 'undead') {
        newCharacter = new Undead(loadGame.compTeam[i].character.level);
      } else if (loadGame.compTeam[i].character.type === 'vampire') {
        newCharacter = new Vampire(loadGame.compTeam[i].character.level);
      }
      newCharacter.attack = loadGame.compTeam[i].character.attack;
      newCharacter.defence = loadGame.compTeam[i].character.defence;
      newCharacter.health = loadGame.compTeam[i].character.health;
      const positionedCharacter = new PositionedCharacter(newCharacter, loadGame.compTeam[i].position);
      this.gameState.compTeam.push(positionedCharacter);
    }
